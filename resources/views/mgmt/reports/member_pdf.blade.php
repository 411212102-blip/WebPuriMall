...
<head>
    <meta charset="utf-8">
    <title>Laporan Aktivitas Member</title>
    <style>
        ...
        .cluster { display: inline-block; padding: 3px 7px; border-radius: 999px; background: #E0F2FE; color: #075985; font-weight: 700; font-size: 8pt; }
        ...
        <div class="head-main">
            <div class="brand">
                ...
                <div class="brand-copy">
                    <h1>Laporan Aktivitas & Segmentasi Member</h1>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th>Email & No HP</th>
                <th class="center">Lifetime Points</th>
                <th class="center">Redeemed</th>
                <th class="center">Saldo Poin</th>
        ...
        <tbody>
            @forelse ($rows as $row)
                <tr>
                    <td>{{ $row->no_pelanggan ?: 'PIM-' . str_pad((string) $row->id_pelanggan, 4, '0', STR_PAD_LEFT) }}</td>
                    <td>{{ $row->nama_pelanggan }}</td>
                    <td>{{ $row->email_pelanggan }}<br>{{ $row->no_whatsapp_pelanggan }}</td>
                    <td class="center">{{ number_format($row->lifetime_points, 0, ',', '.') }}</td>
                    <td class="center">{{ number_format($row->redeemed_points, 0, ',', '.') }}</td>
                    <td class="center">{{ number_format($row->current_points, 0, ',', '.') }}</td>
                    <td class="center"><span class="cluster">{{ $row->nama_cluster }}</span></td>
                    <td class="center">{{ $row->tgl_daftar ? \Carbon\Carbon::parse($row->tgl_daftar)->format('d-m-Y') : '-' }}</td>
                </tr>
            @empty
                <tr><td colspan="8" class="center">Belum ada data member.</td></tr>
            @endforelse
            <tr class="total-row">
                <td>TOTAL KESELURUHAN</td>
                <td></td>
                <td></td>
                <td class="center">{{ number_format($totals['lifetime_points'], 0, ',', '.') }}</td>
                <td class="center">{{ number_format($totals['redeemed_points'], 0, ',', '.') }}</td>
                <td class="center">{{ number_format($totals['current_points'], 0, ',', '.') }}</td>
                <td></td>
                <td></td>
            </tr>
        </tbody>
